...
from tgbot.keyboards.inline import get_show_volume_keyboard
from tgbot.i18n import get_lang, t_bot
...
async def calc_report(message: Message, state: FSMContext):
    ...
    try:
        async with state.proxy() as data:
            ...
            if not s_coords_text:
                try:
                    ...
                    await message.answer(t_bot('load_coords_error', lang))
                    return
        ...
        coords_text = md.text(
            ...
            md.text(esc(t_bot('latitude', lang)), esc(coords[0])),
            md.text(esc(t_bot('longitude', lang)), esc(coords[1])),
            '',
            md.text(bold(s_name[1]), esc(':'), sep=''),
            md.text(esc(t_bot('latitude', lang)), esc(coords[2])),
            md.text(esc(t_bot('longitude', lang)), esc(coords[3])),
            sep='\n',
        )
        ...
        if bot_mode == 0:
            ...
            report_text = md.text(
                ...
                '',
                md.text(
                    esc(t_bot('extra_losses_hca', lang)),
                    esc(f' = {-Lr:.1f} dB'),
                ),
                md.text(
        ...
        if analysis_result and analysis_result.profile_data:
            ...
            volume_text = esc(
                t_bot('volume_data_unavailable', lang)
            )
        ...
        try:
            with open(plot_path, 'rb') as photo:
                await message.bot.send_document(
                    message.chat.id,
                    photo,
                    caption=bold(t_bot('path_profile_caption', lang, name1=s_name[0], name2=s_name[1])),
            ...
            await message.answer(
                t_bot('profile_not_found', lang), reply_markup=get_main_menu(lang)
            )
        finally:
        ...
        if isinstance(e, InvalidResponseException) and "414" in str(e):
            user_message = t_bot('path_too_long', lang)
        elif isinstance(e, APIException):
            user_message = t_bot('elevation_error', lang)
        else:
            user_message = t_bot('occurred_error', lang, error=str(e))

        await message.answer(user_message, reply_markup=get_main_menu(lang))
