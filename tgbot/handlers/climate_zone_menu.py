...
async def show_climate_zone(message: Message, state: FSMContext):
    await message.answer(
        ...
        reply_markup=climate_zone_menu,
    )
...
async def climate_zones(message: Message):
    ...
    try:
        ...
        msg_doc = await message.bot.send_document(
            message.chat.id, file, caption='Карта климатических зон'
        )
        ...
        await message.answer(
            f'Id файла сохранен для более быстрой загрузки.\n'
...
def register_climate_zone_menu(dp: Dispatcher):
    dp.register_message_handler(
        ...
        text=btn_show_climate_zone.text,
        state='*',
    )
    dp.register_message_handler(
        climate_zones,
        ChatTypeFilter(ChatType.PRIVATE),
        text=btn_climate_zones.text,
