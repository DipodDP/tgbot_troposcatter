...
async def calc_t_menu(message: Message, state: FSMContext = '*'):
    ...
    await message.bot.send_message(
        message.chat.id, 'Введите названия точек: ', reply_markup=reply.calc_t_menu
    )
...
async def calc_t_menu_s_names_get(message: Message, state: FSMContext):
    ...
    await message.bot.send_message(
        message.chat.id, 'Добавить еще точку?', reply_markup=inline.add_names
    )
...
async def calc_t_menu_s_coords_get(message: Message, state: FSMContext):
    ...
    await message.answer(
        'Добавить координаты следующей точки?', reply_markup=inline.add_coords
    )
